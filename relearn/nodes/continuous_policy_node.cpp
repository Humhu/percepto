...
public:
	                         ...
	                         ros::NodeHandle& ph )
	{
		...
		_interface.Initialize( lh );
		_manager.Initialize( &_interface, nh, lh );
		_paramNames = _interface.GetParameterNames();

		// Advertise action message topic
		_actionPub = ph.advertise<ContinuousAction::MsgType>( "actions", 0 );
